var functions = {
    ...
    resetModal: ($modal) => {
        ...
        $modal.find('#movie-modal-alert').html('')
    ...
    loadModal: ($modal, data) => {
        ...
        $modal.find('.movie-desc').text(data.overview)
    ...
    alert: ($modal, text, type = 'primary') => {
        ...
        $alert.append(html).slideDown()
    },

    badge: ($modal, text, type = 'primary') => {
        $badge = $modal.find('#movie-modal-badge')
        $badgeText = $modal.find('#movie-modal-badge-text')
        var html = `<span class="badge badge-${type} mr-1">${text}</span>`
        $badgeText.prepend(html)
        $badge.slideDown()
    },
    ...
    request: ($modal, tmdbid) => {
        ...
        $.ajax({
            ...
            if (data.status == 'failed') {
                ...
                movie.badge($modal, 'Monitored')
                $button.slideUp()
                movie.alert($modal, 'This movie is now processing and should be available shortly.', 'success')
            } else {
                //
            }
